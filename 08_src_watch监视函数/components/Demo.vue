...
export default {
  ...
  setup() {
    ...
    //   //此时nv和ov都是数组
    //   console.log('person的age和name发生改变了', nv, ov);
    // });

    //特殊情况
    // watch(() => person.job, (nv, ov) => {
    //   //这里依然无法拿到正确的ov,因为依然监视的是对象
    //   console.log('person的job信息发生了变化',nv,ov);
    // }, {
    //   //注意: 这里必须要加 deep: true
    //   //此处因为监视的是reactive所定义的响应式对象的一个属性(这个属性的值,它依然是一个对象,所以deep配置有效)
    //   deep: true
    // });
